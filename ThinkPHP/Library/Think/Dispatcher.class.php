...
class Dispatcher {
    ...
    static public function dispatch() {
        ...
        if(empty($_SERVER['PATH_INFO'])) {
            ...
            //////---->浩哥修改，开始！
            //由于Dispatcher生成的MODULE_NAME、CONTROLLER_NAME、ACTION_NAME是常量，不能变更，
            //而我们这里需要这几个字符串可以被修改，因此这里采用变量进行重新封装，后续的程序中，
            //MODULE_NAME、CONTROLLER_NAME、ACTION_NAME分别改用$ActionInfo['module']、
            //$ActionInfo['controller']以及$ActionInfo['action']。
            $pathInfo = $_SERVER['PATH_INFO'];
            //加入'73go_action_begin'事件，可以通过勾接此事件，在任何一个Controller将要被实例化
            //之前，可以执行相应的逻辑判断，若有必要导向到另一个处理器（比如没身份需要登录，此时
            //任何的action都被导向到指定的/Home/Login/login这样），则在钩子处理程序中简单地修改
            //传入的参数（$ActionInfo）的module,controller和action就可以轻松实现。
            //
            //当然，若变量赋值不当，会引发错误，这个由行为扩展类自行处理。
            //
            Hook::listen('73go_request_begin', $pathInfo);

            //$varModule = $ActionInfo['module'];
            //$varController = $ActionInfo['controller'];
            //$varAction = $ActionInfo['action'];

            define('__INFO__',trim($pathInfo,'/'));
